...
def train(args, train_dataset, model: PreTrainedModel, tokenizer: PreTrainedTokenizer) -> Tuple[int, float]:
    ...
    for epoch in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if args.model_type == 'syn-bert':
                syn_loss = outputs[1]
                loss = (1 - args.syn_w) * loss + args.syn_w * syn_loss
...
def evaluate(args, model: PreTrainedModel, tokenizer: PreTrainedTokenizer, prefix="") -> Dict:
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            if args.model_type == 'syn-bert':
                syn_loss = outputs[1]
                all_loss += ((1 - args.syn_w) * lm_loss + args.syn_w * syn_loss).item()
                eval_syn_loss += syn_loss.mean().item()
    ...
    if args.model_type == 'syn-bert':
        ...
        eval_syn_loss = eval_syn_loss / nb_eval_steps
        result.update({
            'syn_loss': eval_syn_loss,
            'all_loss': all_loss
        })
    return result
