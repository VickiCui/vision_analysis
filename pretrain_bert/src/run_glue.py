...
from model_for_glue import AutoModelForGLUE
...
def train(args, train_dataset, model, tokenizer):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            inputs['pooling_mode'] = args.pooling_mode
...
def main():
    ...
    parser.add_argument(
        "--pooling_mode", type=str, default='cls', help="the pooling type for getting feature",
    )
    ...
    model = AutoModelForGLUE.from_pretrained(
        args.model_name_or_path,
        from_tf=bool(".ckpt" in args.model_name_or_path),
        config=config,
        cache_dir=args.cache_dir if args.cache_dir else None,
    ...
    # model = ModelForGlue(base_model, config, pooling_mode=args.pooling_mode)
    ...
    if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        model = AutoModelForGLUE.from_pretrained(args.output_dir)
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model = AutoModelForGLUE.from_pretrained(checkpoint)
            model.to(args.device)
