...
from transformers.models.bert.modeling_bert import BertOnlyMLMHead, BertModel, BertPreTrainedModel
...
# class SynBertForMaskedLM(BertPreTrainedModel):
...
#         self.syn_cls = BertOnlyMLMHead(config)
...
#         prediction_scores = self.cls(sequence_output)
#         syn_prediction_scores = self.syn_cls(pooled_output)
#         loss_fct = CrossEntropyLoss()
#         token_loss = loss_fct(prediction_scores.view(-1, self.config.vocab_size), masked_lm_labels.view(-1))

#         if one_hot_labels is None:
#             one_hot_labels = masked_lm_labels_to_one_hot(masked_lm_labels, self.config.vocab_size)

#         # not_masked_pos = (masked_lm_labels == -100).long()
#         # mask_labels = masked_lm_labels_to_one_hot(input_ids * not_masked_pos, self.config.vocab_size, 0)
#         # mask_labels = (mask_labels == 0).long()

#         loss_fct2 = SupConLoss()
#         syn_loss = loss_fct2(syn_prediction_scores.view(-1, self.config.vocab_size), one_hot_labels.view(-1, self.config.vocab_size))

#         return token_loss, syn_loss

# def masked_lm_labels_to_one_hot(masked_lm_labels, class_number, ignore=-100):
#     """Convert masked lm labels to one hot format,

#         Args:
#             masked_lm_labels: labels of shape [bsz, length].
#             class_number: number of class
#         Returns:
#             one_hot_labels: labels of shape [bsz, n_class]
#     """
#     device = masked_lm_labels.device
#     batch_size = masked_lm_labels.size(0)
#     idxs = torch.nonzero(~torch.eq(masked_lm_labels, ignore))
#     value = masked_lm_labels[idxs[:,0], idxs[:,1]]
#     one_hot = torch.zeros(batch_size, class_number)
#     index = (idxs[:,0], value)
